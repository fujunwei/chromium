...
kernel void reorder_shared_texture(texture2d<half, access::read> in[[texture(0)]],
...
kernel void reorder_shared_texture_nonarray(texture2d<half, access::read> in[[texture(0)]],
...
if ((c) < C) {                                          \
  const ushort shared_texture_x = idx / 2; \
  const ushort shared_texture_row = x + y * W; \
  const ushort shared_texture_y = shared_texture_row / 2; \
  const ushort shared_texture_c = (shared_texture_row % 2) * 2; \
  half4 cs = in.read({shared_texture_x, shared_texture_y});       \
