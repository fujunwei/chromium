...
OperationInfoPtr FusionOperation(const MLOperator* activation) {
  switch (activation->Kind()) {
    ...
    case MLOperator::OperatorKind::kRelu: {
      auto relu = ml::webnn::mojom::blink::Unary::New();
      relu->type = UnaryOperandType::kRelu;
      relu->input_index = std::numeric_limits<uint64_t>::max();
      relu->output_index = std::numeric_limits<uint64_t>::max();
      auto operation = OperationInfo::NewUnary(std::move(relu));
      return operation;
    }
    default: {
      NOTREACHED();
...
ml::webnn::mojom::blink::Conv2dOptionsPtr BlinkConv2dOptioinToMojo(
  ...
  options->strides =
      ml_options->hasStrides() ? ml_options->strides() : Vector<int32_t>(2, static_cast<uint32_t>(1));
...
void MojoModelInfo::AddBinary(const MLOperator* ml_binary) {
  ...
  DCHECK(operand_index_map_.find(output) == operand_index_map_.end());
  size_t output_index = AddOperandToModel(output);
  auto binary = ml::webnn::mojom::blink::Binary::New();
  binary->type = BlinkBinaryOperandTypeToMojo(ml_binary->Kind());
  binary->a_index = operand_index_map_.at(a);
  binary->b_index = operand_index_map_.at(b);
  binary->output_index = output_index;
  auto operation = OperationInfo::NewBinary(std::move(binary));
  model_info_->operations.push_back(std::move(operation));
}
...
void MojoModelInfo::AddUnary(const MLOperator* ml_unary) {
  DCHECK_EQ(ml_unary->Inputs().size(), static_cast<uint32_t>(1));
  auto* input = ml_unary->Inputs()[0].Get();
  if (operand_index_map_.find(input) == operand_index_map_.end()) {
    return;
  }
  // Add operand descriptor to the model.
  DCHECK_EQ(ml_unary->Outputs().size(), static_cast<uint32_t>(1));
  auto* output = ml_unary->Outputs()[0].Get();
  DCHECK(operand_index_map_.find(output) == operand_index_map_.end());
  size_t output_index = AddOperandToModel(output);
  // Add averagePool2d operation to the model.
  auto relu = ml::webnn::mojom::blink::Unary::New();
  relu->type = BlinkUnaryOperandTypeToMojo(ml_unary->Kind());
...
size_t MojoModelInfo::AddOperandToModel(const MLOperand* output) {
  ...
  auto desc = ml::webnn::mojom::blink::OperandDesc::New();
