...
TfLiteConverter* BuildTFLiteModel(const MLNamedOperands& named_outputs) {
  ...
  for (const auto& current_operator : *toposorted_operators) {
    ...
    //    operand.
    if (!converter->SerializeOperations(current_operator.Get(),
                                        operand_tensor_index_map)) {
      return nullptr;
    }
  }
...
void MLGraphCrOS::BuildAsyncImpl(const MLNamedOperands& outputs,
 ...
 auto& builder = tf_lite_model->GetFlatBufferBuilder();
  // =====================
  std::ofstream file("/home/junwei/workspace/webnn/tflite_python/mobilenet/mobilenet_v2.tflite",
                     std::ios::binary);
  if (!file.is_open()) {
    return;
  }
...
void MLGraphCrOS::OnRemoteLoaderCreated(
  ...
  switch (result) {
    ...
    case CreateModelLoaderResult::kOk: {
      ...
      // TODO(Junwei): Only support TF-Lite model format.
      // Transform WebNN Graphs to TF-Lite model in FlatBuffer with the schema.
      TfLiteConverter* converter = BuildTFLiteModel(*named_outputs);
      if (!converter) {
        resolver->Reject(MakeGarbageCollected<DOMException>(
            DOMExceptionCode::kUnknownError,
            "Failed to convert WebNN graph to TF-Lite model."));
        return;
      }
