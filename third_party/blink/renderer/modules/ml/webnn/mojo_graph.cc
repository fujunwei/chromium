...
void MojoGraph::BuildSyncImpl(const MLNamedOperands& named_outputs,
                              ExceptionState& exception_state) {
  NOTIMPLEMENTED();
...
ScriptPromise MojoGraph::ComputeImpl(ScriptState* script_state,
  ...
  for (const auto& input : inputs) {
    ...
    auto mapping_iter = inputs_buffer_mapping_.find(input_name);
    memcpy(mapping_iter->value.get(),
  ...
  named_inputs->memory = inputs_buffer_handle_->Clone(
...
void MojoGraph::ComputeSyncImpl(const MLNamedArrayInputs& inputs,
                                const MLNamedArrayOutputs& outputs,
                                ExceptionState& exception_state) {
  NOTIMPLEMENTED();
  exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
...
bool MojoGraph::BuildGraph(BuildRequest* request, String& error_message) {
  size_t shared_buffer_length = 0;
  for (const auto& input : request->inputs_) {
    ...
    inputs_byte_offset_.insert(input->Name(), shared_buffer_length);
    shared_buffer_length += input_byte_length;
  }
  inputs_buffer_handle_ =
      mojo::SharedBufferHandle::Create(shared_buffer_length);
  for (const auto& input : request->inputs_) {
    const String& name = input->Name();
    inputs_buffer_mapping_.insert(
        name, inputs_buffer_handle_->MapAtOffset(inputs_byte_length_.at(name),
                                                 inputs_byte_offset_.at(name)));
  }
  ...
  for (const auto& op : request->sorted_operators_) {
    ...
    if (add_operator_func == nullptr) {
      error_message = "Failed to get function to add the operator";
      return false;
    }
...
void MojoGraph::OnGraphCreated(
  ...
  String error_message;
  if (!BuildGraph(request, error_message)) {
    resolver->Reject(MakeGarbageCollected<DOMException>(
        DOMExceptionCode::kUnknownError, error_message));
    return;
  }
  remote_graph_->BuildGraph(WTF::Bind(&MojoGraph::OnGraphBuilt,
                                      WrapPersistent(this),
