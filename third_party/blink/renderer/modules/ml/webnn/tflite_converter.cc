...
int32_t GetOperatorInputIndex(const MLOperator* op,
  ...
  DCHECK(operand_index_map.Contains(input));
...
int32_t GetOperatorOutputIndex(const MLOperator* op,
  ...
  DCHECK_NE(op, nullptr);
  DCHECK(operand_index_map.Contains(output));
...
TfLiteConverter::TfLiteConverter() {
  ...
  // See more details in
  // https://source.chromium.org/chromium/chromium/src/+/main:third_party/tflite/src/tensorflow/lite/schema/schema_v3.fbs;drc=b009488e56ab7a58b0690f6779353c11e1dfaf0a;l=319
...
bool TfLiteConverter::SerializeOperations(
  ...
  switch (op->Kind()) {
    ...
    case MLOperator::OperatorKind::kConv2d:
      offset = SerializeConv2d(op, operand_index_map);
      break;
    ...
    case MLOperator::OperatorKind::kMaxPool2d:
      offset = SerializePool2d(op, operand_index_map);
      break;
...
OperatorOffset TfLiteConverter::SerializeConv2d(
    ...
    const OperandTensorIndexMap& operand_index_map) {
  // DCHECK_EQ(conv2d->Inputs().size(), 2u);
  auto* input = conv2d->Inputs()[0].Get();
  auto* filter = conv2d->Inputs()[1].Get();
  DCHECK_EQ(conv2d->Outputs().size(), 1u);
  auto* output = conv2d->Outputs()[0].Get();
  if (operand_index_map.find(input) == operand_index_map.end() ||
      operand_index_map.find(filter) == operand_index_map.end() ||
      operand_index_map.find(output) == operand_index_map.end()) {
  ...
  if (options->inputLayout().AsEnum() == V8MLInputOperandLayout::Enum::kNhwc) {
    ...
    uint32_t input_channels = input->Dimensions()[3];
    uint32_t output_channels = output->Dimensions()[3];
    ...
    if (!depthwise) {
      ...
      if (options->filterLayout().AsEnum() !=
        ...
        // error_message = String::Format("The filter layout %s is not
        // supported.",
  ...
  if (options->hasActivation()) {
    switch (options->activation()->Operator()->Kind()) {
      case MLOperator::OperatorKind::kClamp: {
        ...
        uint32_t min =
            clamp_options->hasMinValue()
                ? base::checked_cast<uint32_t>(clamp_options->minValue())
                : 0;
        uint32_t max =
            clamp_options->hasMaxValue()
                ? base::checked_cast<uint32_t>(clamp_options->maxValue())
                : 0;
        if (min == 0 && max == 6) {
  ...
  // TODO::Only support NHWC
  uint32_t output_channels = output->Dimensions()[3];
  int32_t bias_index = conv2d->Inputs().size() == 3
                           ? GetOperatorInputIndex(conv2d, operand_index_map, 2)
                           : SerializeEmptyBuffer(output_channels);
  const std::vector<int32_t> op_inputs = {
      operand_index_map.at(input), operand_index_map.at(filter), bias_index};
  const std::vector<int32_t> op_outputs = {operand_index_map.at(output)};
...
OperatorOffset TfLiteConverter::SerializePool2d(
  ...
  if (dilation_height != 1 || dilation_width != 1) {
    return OperatorOffset();  //"Pool2d in TF-Lite doesn't support dilations.";
  }
  ...
  switch (options->layout().AsEnum()) {
    ...
    case V8MLInputOperandLayout::Enum::kNchw: {
      ...
      // input tensor.
      return OperatorOffset();  //"The nchw input layout is not supported.";
    }
  }
...
OperatorOffset TfLiteConverter::SerializeElementWiseBinary(
  ...
  if (operand_index_map.find(a) == operand_index_map.end() ||
      operand_index_map.find(b) == operand_index_map.end() ||
      operand_index_map.find(output) == operand_index_map.end()) {
    return OperatorOffset();
  }
  tflite::BuiltinOperator binary_op;
  ...
  uint32_t opcode_index =
      base::checked_cast<uint32_t>(model_info_.operator_codes.size()) - 1;
  const std::vector<int32_t> op_inputs = {operand_index_map.at(a),
                                          operand_index_map.at(b)};
  const std::vector<int32_t> op_outputs = {operand_index_map.at(output)};
...
OperatorOffset TfLiteConverter::SerializeRelu(
  ...
  uint32_t opcode_index =
      base::checked_cast<uint32_t>(model_info_.operator_codes.size()) - 1;
  const std::vector<int32_t> op_inputs = {operand_index_map.at(input)};
  const std::vector<int32_t> op_outputs = {operand_index_map.at(output)};
