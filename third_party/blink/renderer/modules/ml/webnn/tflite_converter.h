...
class TfLiteConverter final : public GarbageCollected<TfLiteConverter> {
 public:
  ...
  // Serializes the |buffer| to the FlatBuffer |builder| and returns success.
  // Returns false if the |buffer| could not be serialized because of
  ...
  // Serializes the |input| operand to the FlatBuffer |builder|.
  bool SerializeInput(const MLOperand* input,
                      OperandTensorIndexMap& operand_index_map);
  // Serializes the |output| operand to the FlatBuffer |builder|.
 ...
 private:
  ...
  OperatorOffset SerializeConv2d(
      const MLOperator* conv2d,
      const OperandTensorIndexMap& operand_index_map);
  OperatorOffset SerializePool2d(
  ...
  //           |
  //         output
  // An buffer index starting from 1 that refers to the buffers table which is
