...
template <>
struct TypeConverter<webnn::Pool2dAttributes, blink::MLPool2dOptions*> {
  static webnn::Pool2dAttributes Convert(
    ...
    if (options->hasWindowDimensions()) {
      attributes.window_dimensions =
          mojo::ConvertTo<std::vector<uint32_t>>(options->windowDimensions());
    }
    // If padding is not present, the values are assumed to be [0,0,0,0].
    attributes.padding = mojo::ConvertTo<std::vector<uint32_t>>(
        options->getPaddingOr({0, 0, 0, 0}));
    // If strides is not present, the values are assumed to be [1,1].
    attributes.strides =
        mojo::ConvertTo<std::vector<uint32_t>>(options->getStridesOr({1, 1}));
    // If dilations is not present, the values are assumed to be [1, 1].
    attributes.dilations =
        mojo::ConvertTo<std::vector<uint32_t>>(options->getDilationsOr({1, 1}));
    attributes.auto_pad =
        blink::BlinkAutoPadToComponent(options->autoPad().AsEnum());
    attributes.layout =
    ...
    if (options->hasOutputSizes()) {
      attributes.output_sizes =
          mojo::ConvertTo<std::vector<uint32_t>>(options->outputSizes());
    }
    return attributes;
  }
};
...
template <>
struct TypeConverter<webnn::GemmAttributes, blink::MLGemmOptions*> {
  static webnn::GemmAttributes Convert(const blink::MLGemmOptions* options) {
    CHECK(options);
    webnn::GemmAttributes attributes;
    if (options->hasC()) {
      attributes.c_operand = mojo::ConvertTo<webnn::Operand>(options->c());
    }
...
MLOperand* BuildPool2d(MLGraphBuilder* builder,
  ...
  auto validated_output =
      webnn::ValidatePool2d(mojo::ConvertTo<webnn::Operand>(input),
                            mojo::ConvertTo<webnn::Pool2dAttributes>(options));
...
MLOperand* MLGraphBuilder::gemm(const MLOperand* a,
                                ...
                                ExceptionState& exception_state) {
  auto validated_output = webnn::ValidateGemm(
      mojo::ConvertTo<webnn::Operand>(a), mojo::ConvertTo<webnn::Operand>(b),
      mojo::ConvertTo<webnn::GemmAttributes>(options));
  if (!validated_output.has_value()) {
    exception_state.ThrowDOMException(
        DOMExceptionCode::kDataError,
        WTF::String::FromUTF8(validated_output.error()));
