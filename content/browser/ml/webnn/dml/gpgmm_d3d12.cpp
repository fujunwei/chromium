...
HRESULT STDMETHODCALLTYPE IUnknownImpl::QueryInterface(REFIID riid,
                                                       void** ppvObject) {
...
RESIDENCY_DESC::RESIDENCY_DESC() = default;
...
// static
HRESULT ResidencyManager::CreateResidencyManager(
    const RESIDENCY_DESC& descriptor,
...
HRESULT ResidencyManager::ExecuteCommandLists(
...
ResidencyManager::ResidencyManager(const RESIDENCY_DESC& descriptor)
    : mDevice(std::move(descriptor.Device)),
      mAdapter(std::move(descriptor.Adapter)) {}
...
void ResourceAllocation::Unmap(uint32_t subresource,
...
ALLOCATOR_DESC::ALLOCATOR_DESC() = default;
...
HRESULT ResourceAllocator::CreateAllocator(
  ...
  if (ppResidencyManagerOut != nullptr) {
    ...
    ReturnIfFailed(residencyDesc.Device->CheckFeatureSupport(
    ...
    ReturnIfFailed(ResidencyManager::CreateResidencyManager(residencyDesc,
                                                            &residencyManager));
  }

  Microsoft::WRL::ComPtr<ResourceAllocator> resourceAllocator;
  ReturnIfFailed(CreateAllocator(allocatorDescriptor, residencyManager.Get(),
                                 &resourceAllocator));
...
HRESULT ResourceAllocator::CreateResource(
  ...
  // TODO::Use callback to create ID3D12Pageable
  Microsoft::WRL::ComPtr<ID3D12Pageable> pageable;
  ...
  ReturnIfFailed(Heap::CreateHeap(resourceHeapDesc, mResidencyManager.Get(),
                                  std::move(pageable), &resourceHeap));
  ...
  *ppResourceAllocationOut = new ResourceAllocation(
      allocationDesc, this, resourceHeap, std::move(committedResource));
...
HRESULT ResourceAllocator::CheckFeatureSupport(
...
void ResourceAllocator::DeallocateMemory(
