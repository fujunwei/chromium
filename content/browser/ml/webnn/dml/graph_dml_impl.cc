...
std::vector<UINT> Dimensions(const DML_TENSOR_DESC& tensor_desc) {
...
//     newEdgeInfo->GetTensorDesc() = tensorDesc;
...
#define CREATE_UNARY_OPERATOR(type, inputTensorDesc, dmlOperator)     \
  ...
  operator_desc.InputTensor = &inputTensorDesc;                       \
  operator_desc.OutputTensor = &inputTensorDesc;                      \
...
// output.
#define APPEND_IDENTITY(inputTensorDesc, output_tensor, node) \
...
void GraphDMLImpl::AddElementWiseBinary(uint32_t a_id,
  ...
  auto a_dims = Dimensions(a_node_output->GetTensorDesc());
  auto b_dims = Dimensions(b_node_output->GetTensorDesc());
...
void GraphDMLImpl::AddConv2d(uint32_t input_id,
  ...
  auto input_node = node_output_map_[input_id];
  auto filterEdge = node_output_map_[filter_id];
  ...
  DML_TENSOR_DESC inputTensorDesc = input_node->GetTensorDesc();
  ...
  DML_TENSOR_DESC filterTensorDesc = filterEdge->GetTensorDesc();
  if (options->filterLayout != Conv2dFilterOperandLayout::kOihw) {
    ...
    if (!CreateDmlTensorDesc(mDmlTensorsDesc, filterDmlTensorDesc,
                             &filterEdge->GetTensorDesc(), newFilterDims,
                             newFilterStrides)) {
      DAWN_INTERNAL_ERROR("Failed to create DML tensor description.");
    }
  ...
  std::vector<std::shared_ptr<EdgeInfoBase>> inputEdges = {input_node,
                                                           filterEdge};

  const DML_TENSOR_DESC* biasTensorDescPtr = nullptr;
  DML_TENSOR_DESC newBiasTensorDesc = {};
  if (options->bias_id != 0) {
    assert(node_output_map_.find(options->bias_id) !=
    node_output_map_.end()); auto biasEdge =
    node_output_map_[options->bias_id]; auto biasDims = Dimensions(biasEdge);
    ...
    if (!CreateDmlTensorDesc(mDmlTensorsDesc, biasDmlTensorDesc,
                             &biasEdge->GetTensorDesc(), newBiasDims)) {
      DAWN_INTERNAL_ERROR("Failed to create DML tensor description.");
    }
  ...
  DML_OPERATOR_DESC* fusedActivation =
      CreateFusedOperator(options->activation.get(),
      dmlActicationOperatorDesc,
  ...
  node_output_map_[output_desc->object_id] = output_node;
  return;
}
...
void GraphDMLImpl::AddGemm(uint32_t a_id,
  ...
  // TODO: return directly if BuildResult has error message.
  assert(node_output_map_.find(a_id) != node_output_map_.end());
  auto a_node_output = node_output_map_[a_id];
  auto a_dims = Dimensions(a_node_output);
  assert(node_output_map_.find(b_id) != node_output_map_.end());
  auto b_node_output = node_output_map_[b_id];
  auto b_dims = Dimensions(b_node_output);
  auto output_dims = ConvertDimensions(output_desc->dimensions);
  std::vector<std::shared_ptr<EdgeInfoBase>> inputEdges = {a_node_output,
                                                           b_node_output};

  // The shape of a tensor is 2D definited in WebNN Spec, but DML only
  support
  ...
  if (options->c_id != 0) {
    ...
    auto cEdge = node_output_map_[options->c_id];
    ...
    if (!CreateDmlTensorDesc(mDmlTensorsDesc, cDmlTensorDesc,
                             &cEdge->GetTensorDesc(), cNewDims, cNewStrides))
                             {
      DAWN_INTERNAL_ERROR("Failed to create DML tensor description.");
    }
  ...
  // Reshape back according to output rank if needed to update the output
  edge. if (output_dims.size() < expandedOutputDims.size()) {
    if (!CreateDmlTensorDesc(mDmlTensorsDesc, outputDmlTensorDesc,
                             &a_node_output->GetTensorDesc(), output_dims,
                             {}, true)) {
      DAWN_INTERNAL_ERROR("Failed to create DML tensor description.");
...
void GraphDMLImpl::AddPool2d(uint32_t input_id,
  ...
  if (type == Pool2dType::kAveragePool2d) {
    if (dilations[0] != 1 || dilations[1] != 1) {
      DAWN_INTERNAL_ERROR("The dilations of average pool2d are not
      supported.");
    }
    ...
    WEBNN_CHECK(
        mDevice->CreateOperator(&dmlOperatorDesc,
...
void GraphDMLImpl::AddUnary(uint32_t input_id,
  ...
  switch (type) {
    ...
    case UnaryOperandType::kSigmoid: {
      CREATE_UNARY_OPERATOR(ACTIVATION_SIGMOID, inputTensorDesc,
      dmlOperator);
    } break;
    case UnaryOperandType::kSoftmax: {
      CREATE_UNARY_OPERATOR(ACTIVATION_SOFTMAX, inputTensorDesc,
      dmlOperator);
    } break;
    default:
      assert(0);
...
void GraphDMLImpl::AddOutput(const std::string& name, uint32_t operand_id) {
  ...
  if (node.type == NodeType::kInput || node.type == NodeType::kConstant ||
      strides != nullptr) {
    // auto edge = output_node;
    auto inputTensorDesc = output_node->GetTensorDesc();
    ...
    ComPtr<IDMLOperator> dmlOperator;
    APPEND_IDENTITY(inputTensorDesc, output_tensor, node);
    graph_desc_builder_->Connect({output_node}, {node});
    output_node =
        graph_desc_builder_->CreateNodeOutput(node, 0, output_tensor).get();
  }
