...
// MaybeError GraphDMLNativeImpl::createConstantInput(DML_TENSOR_DESC&
// tensorDESC,
//                                       void const* value,
//                                       size_t size,
...
//   std::shared_ptr<DmlTensorDesc> dmlTensorDesc(new DmlTensorDesc);
//   if (!CreateDmlTensorDesc(mDmlTensorsDesc, dmlTensorDesc, dmlTensorDims,
//                            strides, dataType, tensorFlag)) {
//     DAWN_INTERNAL_ERROR("Failed to create DML tensor description.");
//   }
...
// MaybeError GraphDMLNativeImpl::HardSwish(std::shared_ptr<EdgeInfoBase>&
// inputEdge,
//                             const std::vector<UINT>& inputDims) {
//     dawn::WarningLog() << "The hardSwish is emulated from other operations,
//     maybe the "
...
//         intermediateTensorDesc, inputTensorDesc =
//         inputEdge->outputTensorDESC;

//     std::shared_ptr<DmlTensorDesc> outputDmlTensorDesc(new DmlTensorDesc);
//     if (!CreateDmlTensorDesc(mDmlTensorsDesc, outputDmlTensorDesc,
//                              &inputEdge->outputTensorDESC, inputDims, {},
//                              true)) {
...
//                 .IsError()) {
//             DAWN_INTERNAL_ERROR("Failed to create a constant input
//             tensor.");
//         };
//         CREATE_BINARY_OPERATOR(ADD, inputTensorDesc, constantInputTensorDesc,
//                                outputTensorDesc, dmlOperator);
//         mIntermediateNodesMap[mIntermediateNodes.size()] = dmlOperator;
//         outputEdge = CreateEdgeFromThisNode(outputTensorDesc,
...
//             tensor.");
//         };
//         CREATE_BINARY_OPERATOR(MIN, intermediateTensorDesc,
//         constantSixInputTensorDesc,
//                                outputTensorDesc, dmlOperator);
//         mIntermediateNodesMap[mIntermediateNodes.size()] = dmlOperator;
//         outputEdge = CreateEdgeFromThisNode(outputTensorDesc,
//         mIntermediateNodes.size()); AddEdgesToThisNode({intermediateEdge,
//         mInputs.back()});
//     }
...
//             tensor.");
//         };
//         CREATE_BINARY_OPERATOR(MAX, intermediateTensorDesc,
...
//                                outputTensorDesc, dmlOperator);
//         mIntermediateNodesMap[mIntermediateNodes.size()] = dmlOperator;
//         inputEdge = CreateEdgeFromThisNode(outputTensorDesc,
//         mIntermediateNodes.size());
//         // Reuse the second constant input we created above.
//         AddEdgesToThisNode({intermediateEdge, mInputs[initialInputIndex +
//         2]}); return {};
//     }
// }
...
//             // This value must be in the range [0, InputTensor.DimensionCount
//             - 1]. Negative values
...
//                 std::shared_ptr<DmlTensorDesc> dmlTensorDesc(new
//                 DmlTensorDesc); if (!CreateDmlTensorDesc(
//                         mDmlTensorsDesc, dmlTensorDesc,
//                         &inputEdge->outputTensorDESC,
...
//                     CreateEdgeFromThisNode(outputTensorsDesc[i],
//                     mIntermediateNodes.size(), i);
//             }
...
//         transpose) {
//             assert(transpose->Inputs().size() == 1);
//             const OperandBase* inputOperand = transpose->Inputs()[0].Get();
//             assert(mGraphEdgesMap.find(inputOperand) !=
//             mGraphEdgesMap.end());

//             auto inputDims =
//             ConvertDimensions(transpose->Inputs()[0].Get()->Shape()); auto
//             outputDims =
//             ConvertDimensions(transpose->Outputs()[0].Get()->Shape());
//             std::vector<int32_t> permutation = transpose->GetPermutation();

//             // Transpose need new strides, because the layout has been
//             changed. std::vector<UINT> strides(outputDims.size()),
//             transposedStrides; uint32_t stride = 1; for (size_t i =
//             strides.size(); i-- > 0;) {
//                 strides[i] = stride;
//                 stride *= inputDims[i];
//             }
...
//             DML_TENSOR_DESC outputTensorDesc = {DML_TENSOR_TYPE_BUFFER,
//                                                 &outputDmlTensorDesc->bufferDesc};
//             // Transpose is not a real node in DML, just need to update the
//             edge. mGraphEdgesMap[transpose->PrimaryOutput()] =
//             updateEdge(inputEdge, outputTensorDesc); return {};
//         }
...
//             paddingDims = ConvertDimensions(inputsOperand[1].Get()->Shape());
//             auto outputDims =
//             ConvertDimensions(pad->Outputs()[0].Get()->Shape()); size_t
//             inputRank = inputDims.size();

//             // Workaround(mingming): If padding was added in mGraph, it must
//             be used.
//             // Use "Pad_"+std::to_string(mGraphEdgesMap.size()) to generate a
//             unique name for the
//             // output node. This may be a dml issue:
//             // https://github.com/microsoft/DirectML/issues/133.
//             std::string name = "Pad_" +
//             std::to_string(mGraphEdgesMap.size()); auto paddingTensorDesc =
//             paddingEdge->outputTensorDESC;

//             // Ensure that the DML_TENSOR_FLAGS of output tensor is
//             DML_TENSOR_FLAG_NONE. std::shared_ptr<DmlTensorDesc>
//             outputPaddingTensorDesc(new DmlTensorDesc); if
//             (!CreateDmlTensorDesc(mDmlTensorsDesc, outputPaddingTensorDesc,
//             &paddingTensorDesc,
//                                      paddingDims, {}, true)) {
//                 DAWN_INTERNAL_ERROR("Failed to create DML tensor
//                 description.");
...
//             }
//             mIntermediateNodesMap[mIntermediateNodes.size()] = dmlOperator;
//             auto outputEdge = CreateEdgeFromThisNode(paddingTensorDesc,
//             mIntermediateNodes.size()); AddEdgesToThisNode({paddingEdge});

//             outputEdge->name = name;
//             std::unique_ptr<DML_OUTPUT_GRAPH_EDGE_DESC> outputEdgeDesc(
//                 new DML_OUTPUT_GRAPH_EDGE_DESC);
//             auto outputEdgeInfo =
//             reinterpret_cast<EdgeInfo*>(outputEdge.get());
//             outputEdgeDesc->FromNodeIndex = outputEdgeInfo->nodeIndex;
//             outputEdgeDesc->FromNodeOutputIndex =
//             outputEdgeInfo->outputNodeIndex; outputEdgeDesc->GraphOutputIndex
//             = mOutputs.size();
//             mOutputEdges.push_back({DML_GRAPH_EDGE_TYPE_OUTPUT,
//             outputEdgeDesc.get()});
//             mOutputEdgesDesc.push_back(std::move(outputEdgeDesc));
//             mOutputs.push_back(*outputEdgeInfo);

//             if (mConstantSet.find(inputsOperand[1].Get()) ==
//             mConstantSet.end()) {
//                 DAWN_INTERNAL_ERROR("The padding constant is not
//                 found.");
//             }

//             const op::Constant* paddingConstant =
//                 reinterpret_cast<const
//                 op::Constant*>(inputsOperand[1]->Operator());
...
//                 description.");
//             }
//             outputTensorDesc = {DML_TENSOR_TYPE_BUFFER,
//             &outputDmlTensorDesc->bufferDesc};
...
//                 CreateEdgeFromThisNode(outputTensorDesc,
//                 mIntermediateNodes.size());
//             AddEdgesToThisNode({inputEdge});
//             return {};
//         }

//         MaybeError GraphDMLNativeImpl::AddBatchNorm(const op::BatchNorm*
//         batchNorm) {
//             auto inputs = batchNorm->Inputs();
//             assert(inputs.size() == 3 || inputs.size() == 4 || inputs.size()
//             == 5); assert(mGraphEdgesMap.find(batchNorm->Inputs()[0].Get())
//             != mGraphEdgesMap.end()); auto inputEdge =
//             mGraphEdgesMap[batchNorm->Inputs()[0].Get()]; auto inputDims =
//             ConvertDimensions(inputs[0].Get()->Shape()); auto outputDims =
//             ConvertDimensions(batchNorm->Outputs()[0].Get()->Shape());
//             std::vector<UINT> newInputDims = inputDims, newOutputDims =
//             outputDims, newInputStrides; const BatchNormOptions* options =
//             batchNorm->GetOptions();

//             // When input is a 4-D tensor of the "nchw" or "nhwc" layout,
//             options.axis should be set
...
//             // Reshape 1D mean, variance, scale, bias to 4D with setting 1 to
//             automatically
//             // broadcast.
//             std::vector<DML_TENSOR_DESC> tensorsDesc;
//             std::vector<std::shared_ptr<EdgeInfoBase>> edges;
//             for (size_t i = 1; i < inputs.size(); ++i) {
//                 assert(mGraphEdgesMap.find(batchNorm->Inputs()[i].Get()) !=
//                             mGraphEdgesMap.end());
...
//                     DAWN_INTERNAL_ERROR(
//                         "The 1-D tensor of the values whose length size is
//                         not equal to the size " "of " "the input dimension
//                         denoted by options.axis.");
//                 }
//                 // This tensor's dimensions should be { BatchCount,
//                 ChannelCount, Height,Width}.
//                 // Set 1 to automatically broadcast those dimensions across
//                 the input. std::vector<UINT> expandDims(4, 1);
//                 expandDims[axis] = dims[0];
//                 std::shared_ptr<DmlTensorDesc> dmlTensorDesc(new
//                 DmlTensorDesc); if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//                 dmlTensorDesc, &edge->outputTensorDESC,
//                                          expandDims)) {
//                     DAWN_INTERNAL_ERROR("Failed to create DML tensor
//                     description.");
//                 }
//                 DML_TENSOR_DESC tensorDesc = {DML_TENSOR_TYPE_BUFFER,
//                 &dmlTensorDesc->bufferDesc};
...
//                 if (createConstantInput(constantTensorDesc, &scale,
//                 sizeof(float), {1, 1, 1, 1}, {},
//                                         DML_TENSOR_DATA_TYPE_FLOAT32)
//                         .IsError()) {
//                     DAWN_INTERNAL_ERROR("Failed to create a constant
//                     input tensor.");
//                 };
//                 tensorsDesc.insert(
//                     options->bias == nullptr ? tensorsDesc.end() :
//                     tensorsDesc.begin() + 2, constantTensorDesc);
//                 edges.insert(options->bias == nullptr ? edges.end() :
//                 edges.begin() + 2,
//                              mInputs[index + 1]);
//             }
...
//                 // Create a constant scale.
//                 if (createConstantInput(constantTensorDesc, &bias,
//                 sizeof(float), {1, 1, 1, 1}, {},
//                                         DML_TENSOR_DATA_TYPE_FLOAT32)
//                         .IsError()) {
//                     DAWN_INTERNAL_ERROR("Failed to create a constant
...
//         MaybeError GraphDMLNativeImpl::AddConvTranspose2d(const
//         op::ConvTranspose2d* convTranspose2d) {
//             return DAWN_UNIMPLEMENTED_ERROR("ConvTranspose2D has not been
//             supported on DirectML.");
//         }

//         MaybeError GraphDMLNativeImpl::AddGru(const op::Gru* gru) {
//             return DAWN_UNIMPLEMENTED_ERROR("Gru hasn't been supported on
//             DirectML.");
//         }
...
//         MaybeError GraphDMLNativeImpl::AddReduce(const op::Reduce* reduce) {
//             assert(reduce->Inputs().size() == 1);
...
//             auto inputDims = ConvertDimensions(inputOperand->Shape());
//             auto outputDims =
//             ConvertDimensions(reduce->Outputs()[0].Get()->Shape());

//             auto inputTensorDesc = inputEdge->outputTensorDESC;
//             auto reducedDims = inputDims;
//             for (size_t i = 0; i < options->axesCount; ++i) {
//                 // Axes values must be in the range [0,
//                 InputTensor.DimensionCount - 1].
//                 // The dimensions to reduce where -1 means the last
//                 dimension. uint32_t axis = options->axes[i] == -1 ?
//                 inputDims.size() - 1 : options->axes[i];
//                 axes.push_back(axis);
//                 reducedDims[axis] = 1;
//             }
//             std::shared_ptr<DmlTensorDesc> outputDmlTensorDesc(new
//             DmlTensorDesc); if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//             outputDmlTensorDesc, &inputTensorDesc,
//                                      reducedDims, {}, true)) {
//                 DAWN_INTERNAL_ERROR("Failed to create DML tensor
//                 description.");
//             }
...
//                     CREATE_REDUCE_OPERATOR(L1, inputTensorDesc,
//                     outputTensorDesc, axes, dmlOperator)
//                 } break;
//                 case op::ReduceType::kReduceL2: {
//                     CREATE_REDUCE_OPERATOR(L2, inputTensorDesc,
//                     outputTensorDesc, axes, dmlOperator)
//                 } break;
//                 case op::ReduceType::kReduceMax: {
//                     CREATE_REDUCE_OPERATOR(MAX, inputTensorDesc,
//                     outputTensorDesc, axes,
//                                            dmlOperator)
//                 } break;
//                 case op::ReduceType::kReduceMean: {
//                     CREATE_REDUCE_OPERATOR(AVERAGE, inputTensorDesc,
//                     outputTensorDesc, axes,
//                                            dmlOperator)
//                 } break;
//                 case op::ReduceType::kReduceMin: {
//                     CREATE_REDUCE_OPERATOR(MIN, inputTensorDesc,
//                     outputTensorDesc, axes,
//                                            dmlOperator)
//                 } break;
//                 case op::ReduceType::kReduceProduct: {
//                     CREATE_REDUCE_OPERATOR(MULTIPLY, inputTensorDesc,
//                     outputTensorDesc, axes,
//                                            dmlOperator)
//                 } break;
//                 case op::ReduceType::kReduceSum: {
//                     CREATE_REDUCE_OPERATOR(SUM, inputTensorDesc,
//                     outputTensorDesc, axes,
//                                            dmlOperator)
//                 } break;
//                 default:
//                     DAWN_INTERNAL_ERROR("The reduce op type isn't
//                     supported.");
//             }
//             mIntermediateNodesMap[mIntermediateNodes.size()] = dmlOperator;
//             auto outputEdge = CreateEdgeFromThisNode(outputTensorDesc,
...
//             // Reshape if dimensions needn't be kept. Output edge has been
//             updated with new output
//             // dims.
//             if (!options->keepDimensions) {
//                 if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//                 outputDmlTensorDesc,
//                                          &outputEdge->outputTensorDESC,
//                                          outputDims)) {
//                     DAWN_INTERNAL_ERROR("Failed to create DML tensor
//                     description.");
//                 }
...
//         MaybeError GraphDMLNativeImpl::AddResample2d(const op::Resample2d*
//         resample2d) {
//             assert(resample2d->Inputs().size() == 1);
...
//             auto inputDims = ConvertDimensions(inputOperand->Shape());
//             auto outputDims =
//             ConvertDimensions(resample2d->Outputs()[0].Get()->Shape());
...
//             // Scales is computed by dividing the output sizes by the input
//             sizes.
...
//             }
//             std::vector<float> inputPixelOffsets(4, 0.5),
//             outputPixelOffsets(4, -0.5);
...
//         MaybeError GraphDMLNativeImpl::AddSlice(const op::Slice* slice) {
//             assert(slice->Inputs().size() == 1);
...
//             auto inputDims = ConvertDimensions(inputOperand->Shape());
//             auto outputDims =
//             ConvertDimensions(slice->Outputs()[0].Get()->Shape());
...
//         MaybeError GraphDMLNativeImpl::AddSqueeze(const op::Squeeze* squeeze)
//         {
//             assert(squeeze->Inputs().size() == 1);
...
//             auto inputEdge = mGraphEdgesMap[inputOperand];
//             auto outputDims =
//             ConvertDimensions(squeeze->Outputs()[0].Get()->Shape());
//             // Squeeze perform like reshape which needn't new strides,
//             because the layout has not
//             // been changed.
//             std::shared_ptr<DmlTensorDesc> outputDmlTensorDesc(new
//             DmlTensorDesc); if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//             outputDmlTensorDesc,
...
//             DML_TENSOR_DESC outputTensorDesc = {DML_TENSOR_TYPE_BUFFER,
//                                                 &outputDmlTensorDesc->bufferDesc};
//             // Squeeze is not a real node in DML, just need to update the
//             edge created from it. mGraphEdgesMap[squeeze->PrimaryOutput()] =
//             updateEdge(inputEdge, outputTensorDesc); return {};
//         }

//         MaybeError GraphDMLNativeImpl::AddInstanceNorm(const
//         op::InstanceNorm* instanceNorm) {
//             auto inputs = instanceNorm->Inputs();
//             assert(inputs.size() == 1 || inputs.size() == 2 || inputs.size()
//             == 3);
//             assert(mGraphEdgesMap.find(instanceNorm->Inputs()[0].Get()) !=
//                         mGraphEdgesMap.end());
//             auto inputEdge = mGraphEdgesMap[instanceNorm->Inputs()[0].Get()];
//             auto inputDims = ConvertDimensions(inputs[0].Get()->Shape());
//             auto outputDims =
//             ConvertDimensions(instanceNorm->Outputs()[0].Get()->Shape());
//             std::vector<UINT> newInputDims = inputDims, newOutputDims =
//             outputDims, newInputStrides; const InstanceNormOptions* options =
...
//             std::vector<DML_TENSOR_DESC> tensorsDesc;
//             std::vector<std::shared_ptr<EdgeInfoBase>> edges;
//             // Reshape 1D scale, bias to 4D with setting 1 to automatically
//             broadcast. for (size_t i = 1; i < inputs.size(); ++i) {
//                 assert(mGraphEdgesMap.find(instanceNorm->Inputs()[i].Get())
//                 !=
//                             mGraphEdgesMap.end());
...
//                     DAWN_INTERNAL_ERROR(
//                         "The 1-D tensor of the values whose length size is
//                         not equal to the size " "of " "feature dimension of
//                         the input ");
//                 }
//                 // This tensor's dimensions should be {BatchCount,
//                 ChannelCount, Height, Width}.
//                 // Set 1 to automatically broadcast those dimensions across
//                 the input. std::vector<UINT> expandDims(4, 1); expandDims[1]
//                 = dims[0]; std::shared_ptr<DmlTensorDesc> dmlTensorDesc(new
//                 DmlTensorDesc); if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//                 dmlTensorDesc, &edge->outputTensorDESC,
//                                          expandDims)) {
//                     DAWN_INTERNAL_ERROR("Failed to create DML tensor
...
//             // Set tensor's dimensions to {1, channel, 1, 1} if scale or bias
//             is null. DML_TENSOR_DESC constantTensorDesc; if (options->scale
//             == nullptr) {
...
//             // The input edges order should match the operator's inputs
//             order. return {};
//         }

//         MaybeError GraphDMLNativeImpl::AddConcat(const op::Concat* concat) {
//             assert(concat->Inputs().size() >= 1);
//             auto inputsOperand = concat->Inputs();
//             std::vector<std::shared_ptr<EdgeInfoBase>> inputEdges;
//             std::shared_ptr<EdgeInfoBase> primaryEdge =
//             mGraphEdgesMap[inputsOperand[0].Get()]; auto primaryDims =
//             ConvertDimensions(inputsOperand[0].Get()->Shape());

//             std::vector<DML_TENSOR_DESC> inputTensorsDesc;
//             for (auto& inputOperand : inputsOperand) {
//                 assert(mGraphEdgesMap.find(inputOperand.Get()) !=
//                 mGraphEdgesMap.end()); auto inputEdge =
//                 mGraphEdgesMap[inputOperand.Get()]; auto inputDims =
//                 ConvertDimensions(inputOperand.Get()->Shape());
//                 inputEdges.push_back(inputEdge);

//                 // Expand dimensions to DML_TENSOR_DIMENSION_COUNT_MAX if
//                 needed. if (inputDims.size() <
//                 DML_TENSOR_DIMENSION_COUNT_MAX) {
//                     auto newInputDims = ExpandDimensions(inputDims,
//                     DML_TENSOR_DIMENSION_COUNT_MAX); auto newInputStrides =
//                     CalculateStridesForBroadcast(
//                         inputDims, newInputDims,
//                         inputEdge->outputTensorDESC);
//                     std::shared_ptr<DmlTensorDesc> inputDmlTensorDesc(new
//                     DmlTensorDesc); if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//                     inputDmlTensorDesc,
...
//                         &inputDmlTensorDesc->bufferDesc});
//                 } else if (inputDims.size() ==
//                 DML_TENSOR_DIMENSION_COUNT_MAX) {
//                     inputTensorsDesc.push_back(inputEdge->outputTensorDESC);
//                 } else {
//                     DAWN_INTERNAL_ERROR("The size of input dimensions
//                     is greater than max");
//                 }
//             }

//             auto outputDims =
//             ConvertDimensions(concat->Outputs()[0].Get()->Shape()); auto
//             newOutputDims = outputDims; if (outputDims.size() <
//             DML_TENSOR_DIMENSION_COUNT_MAX) {
//                 newOutputDims = ExpandDimensions(outputDims,
//                 DML_TENSOR_DIMENSION_COUNT_MAX);
//             }
...
//             // Update the axis to align with the
//             DML_TENSOR_DIMENSION_COUNT_MAX. uint32_t axis =
//             concat->GetAxis(); axis += DML_TENSOR_DIMENSION_COUNT_MAX -
//             primaryDims.size();
...
//             // Reshape back according to output rank if needed to update the
//             output edge. if (outputDims.size() < newOutputDims.size()) {
//                 if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//                 outputDmlTensorDesc,
//                                          &primaryEdge->outputTensorDESC,
//                                          outputDims)) {
...
//         MaybeError GraphDMLNativeImpl::AddGemm(const op::Gemm* gemm) {
//             auto inputsOperand = gemm->Inputs();
//             assert(inputsOperand.size() == 2 || inputsOperand.size() == 3);
//             assert(mGraphEdgesMap.find(inputsOperand[0].Get()) !=
//             mGraphEdgesMap.end()); auto aEdge =
//             mGraphEdgesMap[inputsOperand[0].Get()]; auto aDims =
//             ConvertDimensions(inputsOperand[0].Get()->Shape());
//             assert(mGraphEdgesMap.find(inputsOperand[1].Get()) !=
//             mGraphEdgesMap.end()); auto bEdge =
//             mGraphEdgesMap[inputsOperand[1].Get()]; auto bDims =
//             ConvertDimensions(inputsOperand[1].Get()->Shape()); auto
//             outputDims =
//             ConvertDimensions(gemm->Outputs()[0].Get()->Shape());
//             std::vector<std::shared_ptr<EdgeInfoBase>> inputEdges = {aEdge,
//             bEdge};

//             // The shape of a tensor is 2D definited in WebNN Spec, but DML
//             only support 4D,
...
//             std::shared_ptr<DmlTensorDesc> outputDmlTensorDesc(new
//             DmlTensorDesc); if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//             outputDmlTensorDesc, &aEdge->outputTensorDESC,
...
//                 mGraphEdgesMap.end()); auto cEdge =
//                 mGraphEdgesMap[inputsOperand[2].Get()]; auto cDims =
//                 ConvertDimensions(inputsOperand[2].Get()->Shape());
//                 // It is either a scalar, or of the shape that is
//                 unidirectionally broadcastable to
//                 // the shape [M, N] definited in WebNN Spec, DML only support
//                 4D, so broadCast the
//                 // Shape of optional C to {1, 1, M, N } supported in DML.
//                 auto cNewDims = expandedOutputDims;
//                 auto cNewStrides =
//                     CalculateStridesForBroadcast(cDims, cNewDims,
//                     cEdge->outputTensorDESC);
//                 std::shared_ptr<DmlTensorDesc> cDmlTensorDesc(new
//                 DmlTensorDesc); if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//                 cDmlTensorDesc, &cEdge->outputTensorDESC,
//                                          cNewDims, cNewStrides)) {
//                     DAWN_INTERNAL_ERROR("Failed to create DML tensor
//                     description.");
//                 }
//                 cTensorDesc = {DML_TENSOR_TYPE_BUFFER,
//                 &cDmlTensorDesc->bufferDesc}; cTensorDescPtr = &cTensorDesc;
...
//             // Reshape back according to output rank if needed to update the
//             output edge. if (outputDims.size() < expandedOutputDims.size()) {
//                 if (!CreateDmlTensorDesc(mDmlTensorsDesc,
//                 outputDmlTensorDesc,
//                                          &aEdge->outputTensorDESC,
//                                          outputDims, {}, true)) {
...
BuildResult GraphDMLNativeImpl::Finish() {
  if (mInputs.empty()) {
    return BuildResult::kMissingInput;
  }
