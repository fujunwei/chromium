...
#ifndef CONTENT_BROWSER_ML_WEBNN_DML_GPGMM_D3D12_H_
#define CONTENT_BROWSER_ML_WEBNN_DML_GPGMM_D3D12_H_

// GPGMM minimum viable implementation (MVI).
//
// GPGMM MVI allows users to leverage GPGMM's portable GMM interface without
// requiring to build the full GPGMM implementation, allowing for incremental
// enabling during development.
//
// GPGMM MVI specifically,
// * Is not thread-safe.
// * Is functionally-equivelent to calling
// ID3D12Device::CreateCommittedResource.
// * Does not perform residency management.
//
// User should decide to define the following macros:
// - GPGMM_D3D12_HEADERS_ALREADY_INCLUDED: D3D12 platform headers will be
// already included before this header and does not need to be re-included.
// - GPGMM_WINDOWS_HEADERS_ALREADY_INCLUDED: Windows.h will be already included
// before this header and does not need to be re-included.
//
...
class Heap final : public MemoryBase, public IUnknownImpl {
 ...
 private:
  explicit Heap(Microsoft::WRL::ComPtr<ID3D12Pageable> pageable,
...
class ResidencyManager final : public IUnknownImpl {
 public:
  static HRESULT CreateResidencyManager(
...
class ResourceAllocation final : public MemoryAllocation, public IUnknownImpl {
 public:
  ...
  void Unmap(uint32_t subresource = 0,
             const D3D12_RANGE* pWrittenRange = nullptr);
 ...
 private:
  ...
  explicit ResourceAllocation(const RESOURCE_ALLOCATION_DESC& desc,
...
class ResourceAllocator final : public MemoryAllocator, public IUnknownImpl {
 public:
  ...
  static HRESULT CreateAllocator(
...
#endif  // CONTENT_BROWSER_ML_WEBNN_DML_GPGMM_D3D12_H_
